...
contract BuildConfigurePoolCalldata is Script {
    function run() external view returns (bytes memory data) {
        ...
        IERC20 cheap = IERC20(vm.envAddress("CHEAP_TOKEN_ADDRESS"));
        data = abi.encodeCall(CheapFeeSplitter.configurePool, (feeManager, poolId, cheap));
        console2.log("Fee manager", feeManager);
        console2.log("CHEAP token", address(cheap));
